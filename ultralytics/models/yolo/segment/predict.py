...
class SegmentationPredictor(DetectionPredictor):
    ...
    def construct_result(self, pred, img, orig_img, img_path, proto):
        ...
        if not len(pred):  # save empty boxes
            ...
            masks = ops.process_mask_native(proto, pred[:, 6:], pred[:, :4], orig_img.shape[:2],box_margin_factor=self.args.box_margin_factor,box_max_margin=self.args.box_max_margin)  # HWC
        else:
            masks = ops.process_mask(proto, pred[:, 6:], pred[:, :4], img.shape[2:], upsample=True, box_margin_factor=self.args.box_margin_factor, box_max_margin=self.args.box_max_margin)  # HWC
            pred[:, :4] = ops.scale_boxes(img.shape[2:], pred[:, :4], orig_img.shape)
